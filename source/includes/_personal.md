...
В этом разделе можно установить фильтрацию IP-адресов, при работе с API, и просмотреть отчтеты об авторизациях. 

<aside class="warning">
        Не меняйте способ получения кода подтверждения - по умлочанию установлено **Email**. Сообщения приходят на адресс указаный в профиле.
    </aside>
### Настройки

**Фильтрация IP-адресов** работает по принципу **белого списка** - запросы будут приниматься только с указанного IP-адреса. Функция активируется чекбоксом: </br>
**<input type="checkbox" id="filter" name="filter">  <label for="filter">Использовать фильтрацию по IP-адресу при использовании API</label>**
